            ...
            {isAuthenticated ? (
              <>
                ...
                <Link href="/create" className="hidden md:block">
                  <Button variant="primary" size="sm" className="h-10 px-6 rounded-full shadow-lg shadow-primary/20 hover:shadow-primary/30 text-sm font-semibold tracking-wide">
              ...
              <>
                ...
                <Link href="/login" className="hidden md:block">
                  <Button variant="ghost" size="sm" className="h-10 px-5 rounded-full text-sm font-medium hover:bg-gray-50 text-gray-600">
                    {tLogin('signIn')}
                  </Button>
                </Link>
                
                <Link href="/register" className="hidden md:block">
                  <Button variant="primary" size="sm" className="h-10 px-6 rounded-full shadow-lg shadow-primary/20 hover:shadow-primary/30 text-sm font-semibold">
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-1">
              ...
              {!isAuthenticated && (
                <>
                  ...
                  <Link
                    href="/register"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center gap-3 px-4 py-3 rounded-xl text-base font-semibold text-primary hover:bg-primary/5 transition-colors min-h-[48px]"
                  >
                    {tRegister('createAccount')}
                  </Link>
                </>
              ...
              >
                ...
                <span>{locale === 'en' ? 'English' : 'Русский'} → {locale === 'en' ? 'Русский' : 'English'}</span>
              </button>
            </nav>
          </div>
        </>
...
export const Header: React.FC = () => {
  return (
    <Suspense fallback={<header className="fixed top-0 left-0 right-0 z-50 h-20 bg-white/80 backdrop-blur-md border-b border-gray-100/50" />}>
      <HeaderContent />
    </Suspense>
  );
};

...
function HomePageContent() {
  ...
  const handleQuickBetSubmit = (e: React.FormEvent) => {
    ...
    if (!quickBetForm.betAmount || parseFloat(quickBetForm.betAmount) <= 0) errors.betAmount = t('createBetForm.errors.poolAmountPositive');
    if (!quickBetForm.coefficient || parseFloat(quickBetForm.coefficient) <= 1) errors.coefficient = t('createBetForm.errors.coefficientInvalid');
  ...
  const filteredBets = useMemo(() => {
    ...
    if (dateRange) {
      result = result.filter((bet) => {
        ...
        switch (dateRange) {
          case "today": return isToday(createdAt);
          case "week": return isThisWeek(createdAt);
          case "month": return isThisMonth(createdAt);
          default: return true;
        }
      });
    }
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case "popularity": return (b.betsCount + b.likesCount) - (a.betsCount + a.likesCount);
        case "coefficient": return b.coefficient - a.coefficient;
        case "pool": return b.betAmount - a.betAmount;
        default: return 0;
      }
    });
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 items-center mb-20">
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-100 shadow-lg">
              ...
              {!isAuthenticated && (
                <p className="mb-5 text-sm text-gray-600 bg-gray-50 rounded-xl p-3">{t('createBetForm.registerPrompt')}</p>
              )}
              ...
              <form onSubmit={handleQuickBetSubmit} className="space-y-6" noValidate>
                ...
                <Button type="submit" variant="primary" className="w-full h-11 rounded-lg font-semibold">
                  {isAuthenticated ? t('createBetForm.createButton') : t('createBetForm.registerButton')}
